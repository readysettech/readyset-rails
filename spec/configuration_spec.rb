...
RSpec.describe Readyset::Configuration do
  context 'when database.yml present' do
    it 'defaults to the url defined in config/database.yml' do
      ...
      allow(YAML).to receive(:load).and_return({ 'test' => { 'readyset' => { 'url' => 'test_db_url' } } })
  ...
  context 'when neither config/database.yml nor ENV provides a database_url' do
    it 'logs an error' do
      ...
      expect(Rails.logger).to have_received(:error).with('Failed to set database URL: No valid configuration found in config/database.yml or ENV')
    end
  end
end
